...
if not os.path.exists(filenameResults):
    ...
    f.write("No.\tset Temp./K\ttime/s\tdt(Kei2000)/volts\tdt(Kei2182A)/volts\tTemp(Kei2182A)/K\tHeat or cool\tRun\tDate\tTime of Day\tSampleName\tTpv(Chino)\n")
    f.close()
...
def get_measurement_data():
    ...
    測定データを取得する関数
    Returns:
        tuple: (pv2000, pv2182A, Tpv2000, Tpv2182A, Tpvchino)
    """
    try:
        ...
        return pv2000, pv2182A, Tpv2000, Tpv2182A, Tpvchino
...
def record_and_display_measurement(list_pointer, t0, t1, Tsvtemp, pv2000, pv2182A, Tpv2182A, Tpvchino, 
                                 status, run_number, sampleName, filenameResults):
    """
    測定結果の表示と記録を行う関数
    Args:
        list_pointer: 測定回数
...
# ヘッダーの表示
header = "No.  Run  Date       Time     t/s    Tsv/K    pv2000    pv2182A    Tpv2182    Tpvchino"
print(header)
...
while time.time() - t0 < wait1st:
    ...
    try:
        t1 = time.time()
        t2 = t1-previous_measurement_time
        previous_measurement_time = t1
        
        # 測定値の取得
        pv2000, pv2182A, Tpv2000, Tpv2182A, Tpvchino = get_measurement_data()
        
        # 結果の表示と記録
        list_pointer += 1
        record_and_display_measurement(
            list_pointer, t0, t1, Tsvtemp, pv2000, pv2182A, Tpv2182A, Tpvchino,
            "waiting", 0, sampleName, filenameResults
        )
...
for k in range(1,len(line)):
    ...
    while True:
        ...
        record_and_display_measurement(
            list_pointer, t0, t1, Tsvtemp, pv2000, pv2182A, Tpv2182A, Tpvchino,
            status, k, sampleName, filenameResults
        )
